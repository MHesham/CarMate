...
static char pcAssertBuffer[128];
static char pcLineNumberBuffer[8];

void vAssertCalled( PGM_P pcExpression, PGM_P const pcFileName,
					const char * const pcFunctionName, unsigned long ulLine ) __attribute__((weak));
					...
					const char * const pcFunctionName, unsigned long ulLine )
{
	...
	*pC = '\0';
	...
	utoa(ulLine, pcLineNumberBuffer, 10);

	strcat_P( pC, PSTR("\r\nASSERT:"));
	strcat_P( pC, pcExpression );
	strcat_P( pC, PSTR("["));
	strcat_P( pC, pcFileName );
	strcat_P( pC, PSTR(":"));
	strcat( pC, pcFunctionName );
	strcat_P( pC, PSTR(","));
	strcat( pC, pcLineNumberBuffer );
	strcat_P( pC, PSTR("]\r\n"));
